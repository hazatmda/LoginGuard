...
final class AttemptsModel extends ListModel
{
    ...
    protected function populateState($ordering = 'created', $direction = 'DESC'): void
    {
        ...
        $this->setState('filter.status', (string) ($filters['status'] ?? $app->getUserStateFromRequest($this->context . '.filter.status', 'filter_status', '', 'cmd')));
        $this->setState('filter.where_at', (string) ($filters['where_at'] ?? $filters['client'] ?? $app->getUserStateFromRequest($this->context . '.filter.where_at', 'filter_where_at', '', 'cmd')));
    ...
    protected function getListQuery()
    {
        ...
        if ($whereAt !== '') {
            $query->where('(' . $db->quoteName('where_at') . ' = ' . $db->quote($whereAt)
